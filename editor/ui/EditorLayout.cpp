...
float EditorLayout::extractFloat(const std::string& json, const std::string& key,
    ...
    catch (const std::exception& e) { std::cerr << "[EditorLayout] Failed to parse float for key '" << key << "': " << e.what() << std::endl; return defaultVal; }
}
...
int EditorLayout::extractInt(const std::string& json, const std::string& key,
    ...
    catch (const std::exception& e) { std::cerr << "[EditorLayout] Failed to parse int for key '" << key << "': " << e.what() << std::endl; return defaultVal; }
}
